...
import { useMerges } from '@/lib/hooks/use-admin-moderation';
import { MergeSideBySide } from '@/components/moderation/merge-side-by-side';
...
export default function MergesModerationPage() {
  const t = useT();
  const [page, setPage] = useState(1);
  const { data, isLoading } = useMerges(page, PAGE_SIZE);
    ...
    <>
      <div className="space-y-4">
        {data.items.map((proposal) => (
          <MergeSideBySide key={proposal.id} proposal={proposal} />
        ))}
      </div>
